...
def find_outliers(data: np.ndarray):
    """Returns indices where values more than 2 standard deviations from mean"""
    out = np.where(np.abs(data - data.mean()) > 2 * data.std())
    # np.where return tuple of all dimensions of the data. We want the first
    return out[0]


class OutliersServer(OutliersServicer):
    def Detect(self, request, context):
        logging.info(f'detect request size {len(request.metrics)}')
        ...
        logging.info(f'found {len(indices)} outliers')
